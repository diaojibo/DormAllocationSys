...
public class Dorminfo extends HttpServlet {
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user==null){
			...
			try {
					...
					sql="select * from stud_dorm where dorm_id='"+dorm_id+"'";
					rs=stmt.executeQuery(sql);
					ArrayList result=new ArrayList();
					while(rs.next()){
						String stid=rs.getString("stu_id");
						sql="select * from student where id='"+stid+"'";
						ResultSet rst=stmt2.executeQuery(sql);
						rst.next();
						ArrayList t=new ArrayList();
						t.add(rst.getString("name"));
						t.add(rst.getString("id"));
						t.add(rst.getString("class"));
						t.add(rst.getString("dept"));
						t.add(rst.getString("l_tel"));
						t.add(rst.getString("s_tel"));
						result.add(t);
					}
