...
public class ThingsUpdate extends HttpServlet {
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user==null){
			...
			try {
					...
					String newthings=(String)req.getParameter("things");
...
//					String newtime = formatter2.format(new Date());
					...
					String sql="insert into bring_out(stu_id,date,name)"
					+" values('"+stuid+"','"+newdate+"','"+newthings+"');";
					stmt.executeUpdate(sql);
					con.close();
					tool.right("登记成功!");
					//RequestDispatcher rd=req.getRequestDispatcher("MaintainInfo");
					out.println("<script>location.href='things.jsp'</script>");
					//rd.forward(req, resp);
				} catch (Exception e) {
