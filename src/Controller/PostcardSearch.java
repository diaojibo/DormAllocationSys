...
public class PostcardSearch extends HttpServlet {
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user==null){
			...
			try {
					...
					String sql="select * from stud_post";
					rs=stmt.executeQuery(sql);
					ArrayList result=new ArrayList();
					while(rs.next()){
						String stu_id=rs.getString("stu_id");
						String t_date=rs.getString("date");
						String t_sendname=rs.getString("send_name");
					...
					if(result!=null){
			                for(int i=0;i<result.size();i++){
			                	...
			                	out.println("<td>收件人："+temp2.get(1)+"</td>");
			                	out.println("<td>寄信人："+temp2.get(2)+"</td>");
			                	out.println("<td>日期："+temp2.get(3)+"</td>");
			                	out.println("</tr>");
			                }
			            }
