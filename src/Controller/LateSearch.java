...
public class LateSearch extends HttpServlet {
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user==null){
			...
			try {
					...
					String sql="select * from stay_o_late";
					rs=stmt.executeQuery(sql);
					ArrayList result=new ArrayList();
					while(rs.next()){
						String stu_id=rs.getString("stu_id");
						String t_reason=rs.getString("reason");
						String t_counsellor=rs.getString("counsellor");
					...
					if(result!=null){
			                for(int i=0;i<result.size();i++){
			                	...
			                	out.println("<td>学号："+temp2.get(0)+"</td>");
			                	out.println("<td>姓名："+temp2.get(1)+"</td>");
			                	out.println("<td>宿舍："+temp2.get(2)+"</td>");
			                	out.println("<td>辅导员："+temp2.get(3)+"</td>");
			                	out.println("<td>理由："+temp2.get(4)+"</td>");
			                	out.println("<td>日期："+temp2.get(5)+"</td>");
			                	out.println("</tr>");
			                }
			            }
